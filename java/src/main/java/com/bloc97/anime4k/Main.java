...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length < 2) {
            System.out.println("Error: Please specify input and output folder.");
            return;
        }
        String myDirectoryPath = args[0];
        String outDirectoryPath = args[1];
        ...
        if (directoryListing != null) {
            for (File child : directoryListing) {
                ...
                ImageIO.write(img, "png", new File(outDirectoryPath + '/' + child.getName()));
